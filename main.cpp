...
constexpr int N_x = 320, N_y = 320, N_z = 320;
...
public:
	...
	int grid_idx_from_coords(int x, int y, int z) const
	{
	...
	void fill_grid(int size)
	{
		...
		if (comm_rank == RANK_ROOT)
		{
			for (int i = 0; i < z_size; i++)
			{
				...
				{
					grid[grid_idx_from_coords(0, j, i)] = phi(-1, j, i);
				}
			}
		}
		if (comm_rank == (size - 1))
		{
			for (int i = 0; i < z_size; i++)
			{
				for (int j = 0; j < y_size; j++)
				{
					grid[grid_idx_from_coords(x_size - 1, j, i)] = phi(x_size - 2, j, i);
				}
			}
		}
	}
	...
	void calculate_grid_center()
	{
		for (int i = 2; i < x_size - 2; ++i)
		{
			for (int k = 0; k < z_size; ++k)
			{
				for (int j = 0; j < y_size; ++j)
				{
					calculate_next_phi(i, j, k);
				}
			}
		}
	}

	void calculate_grid_borders()
	{
		for (int k = 0; k < z_size; ++k)
		{
			for (int j = 0; j < y_size; ++j)
			{
				calculate_next_phi(1, j, k);
				calculate_next_phi(x_size - 2, j, k);
			}
		}
	}


	void calculate_next_phi(int i, int j, int k)
	{
		double left_x_neighbor = grid[grid_idx_from_coords(i - 1, j, k)];
		double right_x_neighbor = grid[grid_idx_from_coords(i + 1, j, k)];
		double backw_y_neighbor, forw_y_neighbor;

		if (j >= (y_size - 1)) {
			backw_y_neighbor = phi(i - 1, j + 1, k);
		} else {
			backw_y_neighbor = grid[grid_idx_from_coords(i, j + 1, k)];
		}
		if (j <= 0) {
			forw_y_neighbor = phi(i - 1, -1, k);
		} else {
			forw_y_neighbor = grid[grid_idx_from_coords(i, j - 1, k)];
		}

		double up_z_neighbor, down_z_neighbor;
		if (k >= (z_size - 1)) {
			up_z_neighbor = phi(i - 1, j, k + 1);
		} else {
			up_z_neighbor = grid[grid_idx_from_coords(i, j, k + 1)];
		}
		if (k <= 0) {
			down_z_neighbor = phi(i - 1, j, -1);
		} else {
			down_z_neighbor = grid[grid_idx_from_coords(i, j, k - 1)];
		}
		...
		next_phi[grid_idx_from_coords(i, j, k)] = coef * (tmp - 6 + a * phi(i - 1, j, k));
		...
		for (int k = 1; k < x_size - 1; k++)
		{
			for (int j = 0; j < z_size; j++)
			{
				for (int i = 0; i < y_size; i++)
				{
					...
					double delta = fabs(grid[grid_idx_from_coords(k, i, j)] - phivar);
